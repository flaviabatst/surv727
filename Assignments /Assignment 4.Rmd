...
SELECT 2016, COUNTIF(*) as count_crime
FROM crime

#there are 7668437 rows in the table "crime". 
```
...
```{sql connection= con}
SELECT 2016, COUNT(primary_type) as arrest_primary
FROM crime
...
```{sql connection= con}
SELECT 2016, primary_type, EXTRACT(HOUR FROM date) 
FROM crime
...
SELECT primary_type,
  COUNTIF(primary_type = "HOMICIDE") as count_homicide
FROM crime
GROUP BY year
...
```{sql connection= con}
SELECT primary_type,
   COUNTIF(year = 2015) AS arrest_2015,
      COUNTIF(year = 2016) AS arrest_2016,
FROM crime
GROUP BY district
ORDER BY COUNT(primary_type) DESC
```
